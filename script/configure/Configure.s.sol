...
contract Configure is ConfigureBase {
    function run(uint256[] memory peerChainIds_) external {
        ...
        for (uint256 i; i < peersCount_; i++) {
            ...
            (address peerBridge_, address peerMToken_,,,, address peerWrappedM) = _readDeployment(peerChainId);
            peers_[i] = PeerConfig({ chainId: peerChainId, bridge: peerBridge_, mToken: peerMToken_, wrappedM: peerWrappedM });
        }

        vm.startBroadcast(deployer_);

        _configurePeers(portal_, mToken_, wrappedM_, bridge_, peers_);
